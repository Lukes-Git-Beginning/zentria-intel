#!/usr/bin/env python3
"""
Morning-Pulse script for 2026-07-21 (Tuesday)
...
# Tuesday tier-2 rotation: helpdesk, crm-core (but we focus on tier1 today)
# Watermarks
...
# Keywords use word-boundary patterns to avoid false positives.
# Compiled at module load time.
def _make_kw_patterns(kw_list):
    """Compile keyword list to word-boundary regex patterns."""
    compiled = []
    for kw in kw_list:
        # Use word boundaries for short keywords, substring for longer phrases
...
def simhash64(text: str) -> int:
    """Simple 64-bit simhash on the text."""
...
def module_match(item: dict) -> tuple[list[str], float]:
    """Return (matched_modules, score_factor)."""
    text = (item.get("title") or "") + " " + (item.get("summary") or "")
    # Also include source module hint as a strong signal
    ...
    for mod, patterns in MODULE_KEYWORDS.items():
        for pat in patterns:
            if pat.search(text):
                matched.append(mod)
                break
    # Always include module_hint if it's a known module
...
def recency_decay(published_at: str) -> float:
    ...
    try:
        ...
        if age_hours < 0:
            age_hours = 0
        # Linear decay: 1.0 at 0h, 0.5 at 48h
...
SOURCES = [
    ...
    # BSI: only Cosmi-stack-relevant advisories (postgres, cloud, saas, go, python, nginx, grafana, docker, kubernetes)
...
for src in SOURCES:
    ...
    if src["type"] == "hn-json":
        # HackerNews: fetch top story IDs, then fetch each story
    ...
    # Per-source max items cap
    max_items = src.get("max_items", 50)
    # Optional cosmi-stack filter (for BSI-style sources)
    ...
    for item in parsed:
        ...
        # Apply cosmi_stack_filter if set
        if cosmi_filter:
...
def passes_prefilter(item: dict) -> bool:
    ...
    # Title word count + module keyword check
...
# Count n_sources per title-cluster (cross-source corroboration).
# Use first 6 words of title as key, count distinct source_ids mentioning it.
...
for item in filtered:
    ...
    # Cap mod_score contribution: if only module_hint and no keyword match, use 0.25 base
    if mods and mods[0] == item.get("module_hint") and len(mods) == 1:
        effective_mod_score = 0.25  # source-assigned module only, no keyword match
    elif not mods:
        effective_mod_score = 0.1   # no module match at all
    else:
...
for item in filtered[:60]:  # Top 60 for breakdown
    mods = item.get("matched_modules", [])
    # Primary module from module_hint or first matched
    ...
    if primary and primary in module_buckets:
        if len(module_buckets[primary]) < 5:  # Cap per module
...
muted_count = len(MUTED_SOURCES)  # from settings.yaml muted_sources list
total_sources_polled = len(SOURCES)  # sources we attempted
...
for mod_id, label in MODULE_LABELS.items():
    ...
    # Also check items matched to this module (even if primary differs)
