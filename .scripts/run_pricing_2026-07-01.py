#!/usr/bin/env python3
"""
Monthly pricing diff — 2026-07-01
Scrapes all competitor pricing pages, diffs against June 2026 baseline,
writes monthly/2026-07-pricing.md and updates snapshots.
"""
...
REPORT_FILE = f"{WORKDIR}/monthly/2026-07-pricing.md"
...
BERLIN_TZ = timezone(timedelta(hours=2))  # CEST July
...
COMPETITORS = [
    ...
    },
    {
        "name": "Bexio",
        "slug": "bexio",
        "pricing_url": "https://www.bexio.com/de-DE/preise",
        "region": "dach",
        "threat_level": "high",
...
def extract_pricing_section(markdown_text, competitor_name):
    ...
    # Score each line for pricing relevance
    ...
    # Find clusters of high-scoring lines
    ...
    # Expand context around high-score lines (±5 lines)
    keep = set()
    ...
    # Hard cap at ~8000 chars to keep diffs manageable
    if len(result) > 8000:
...
def compute_diff(old, new, slug):
    ...
    diff = list(difflib.unified_diff(
        old_lines, new_lines,
        fromfile=f"{slug}.md (Juni 2026)",
        tofile=f"{slug}.md (Juli 2026)",
        n=3,
    ))
    ...
    diff_text = ''.join(diff[:200])  # cap at 200 diff lines
...
def interpret_change(competitor_name, slug, diff_text, old_snapshot, new_snapshot):
    ...
    if old_prices and new_prices:
        try:
            ...
            if new_avg > old_avg * 1.03:
                ...
                cosmi_implications.append(f"Preisliche Distanz zu Cosmi-Modulen hat sich verbessert — Positionierungs-Update prüfen.")
                ...
                cosmi_implications.append(f"Preisdruck steigt. Cosmi-Differenzierung über Features/Compliance statt Preis betonen.")
        except Exception:
            pass
    ...
    if new_tiers:
        ...
        cosmi_implications.append(f"Tier-Segmentierung im Markt verdichtet sich. Cosmi-Tier-Struktur abgleichen.")
    if removed_tiers:
...
def main():
    ...
    print(f"=== intel-monthly-pricing 2026-07-01 ===")
    ...
    for comp in COMPETITORS:
        ...
        # Check for actual block pages (not CDN references)
        block_signals = [
        ...
        # Only flag as blocked if we see specific block text OR page is suspiciously tiny
        ...
        if blocked:
            ...
            # Load old snapshot for continuity
        ...
        time.sleep(1.2)  # polite delay between scrapes

    # -------------------------------------------------------------------------
    # Build Report
    # -------------------------------------------------------------------------
    checked_ok = [r for r in results if r.get("status") in ("ok",)]
...
def build_report(results, changes, checked_ok, checked_blocked, checked_error,
    ...
    lines.append("# Pricing-Diff Juli 2026")
    lines.append("")
    ...
    if n_changes == 0:
        ...
        lines.append("")
        lines.append("> Kein einziger Konkurrent hat im Juli 2026 seine Pricing-Seite signifikant geändert. "
    ...
    if ai_changes:
        ...
        observations.append(
            "**AI-Bundling-Stabilität:** Kein Konkurrent hat diesen Monat neue AI-Pricing-Konditionen eingeführt. Der AI-Bundling-Trend aus dem Juni (monday.com AI-Credits) hat sich nicht weiter ausgedehnt."
        )
    ...
    if not dach_changes:
        observations.append(
            f"**DACH-Spezialisten stabil:** {', '.join(r['name'] for r in dach_ok)} zeigen keine Pricing-Änderungen. "
            "DACH-Segment bleibt im Sommer preislich ruhig — typisches Muster außerhalb von Herbst-Relaunch-Wellen."
        )
    else:
        observations.append(
            f"**DACH-Bewegung:** {len(dach_changes)} DACH-Konkurrent(en) mit Änderungen: "
            f"{', '.join(r['name'] for r in dach_changes)}. Mögliches Q3-Repositioning."
        )

    # Blocked competitors note
    if checked_blocked:
        blocked_names = ', '.join(r['name'] for r in checked_blocked)
        observations.append(
            f"**Bot-Schutz:** {blocked_names} blockieren weiterhin Headless-Scraping. "
            "Manuelle Quartalsprüfung oder Session-basiertes Scraping notwendig für vollständige Coverage."
        )
    ...
    if n_changes == 0:
        observations.append(
            "**Markt-Stabilität:** Kein Konkurrent hat im Juli Pricing geändert. Typisch für Sommermonat — "
            "Relaunch-Wellen meist September/Oktober. Nächste Review-Intensivierung für Herbst einplanen."
        )
    ...
    lines.append(f"next_run:             2026-08-01T04:00:00Z")
    lines.append("```")
    lines.append("")
